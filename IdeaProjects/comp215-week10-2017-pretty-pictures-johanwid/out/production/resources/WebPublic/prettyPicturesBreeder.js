...
/**
 * Author: Tim Van Baak (tev2)
 * Based in part on previous version by Clayton Drazner and Matthew Kindy II
 */
...
(function() {
    ...
    window.onload = function() {
        ...
        $("#buttonTest").on("click", buttonTest);
        ...
        $("#buttonTest").prop("disabled", false);
        ...
        $.ajax({
            ...
            success: function(data) {
                console.log("/init/ succeeded: " + data);

                // Ensure that response is well-formed
                data = JSON.parse(data);
                if (!validate(data, "/init/")) {
                    return;
                }
    ...
    function displayGenImages(genNum, numImages) {
        ...
        for (var i = 0; i < numImages; i++) {
            ...
            // Add the image
            var imgSrc = "/image/gen/" + genNum + "/img/" + i + "/height/200/width/200/";
            ...
            zoomBtn.zoomSrc = "/image/gen/" + genNum + "/img/" + i + "/height/800/width/800/";
            zoomBtn.innerHTML = "&#128269;";
            zoomBtn.onclick = function() {
                ...
                img.onclick = function() {window.open(this.src);};  // Clicking opens the image in full
    ...
    function buttonBreed() {
        ...
        if (selected.length > 1) {  // If there are enough selected images
            ...
            selected.each(function() { url = url + this.imgIndex + "/"; });
