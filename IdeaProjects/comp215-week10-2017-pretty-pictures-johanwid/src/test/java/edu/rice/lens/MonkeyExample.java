 ...
 *
 * <p>This class includes some nested data classes. Monkeys have names and hats. Those hats have colors and feathers.
 * The feathers also have a color. You can see that each class also defines a series of lenses. So the Monkey class
 * defines two lenses: Hat and Name, each of which have getters and functional setters. These lenses can be composed
 * with other lenses, which you can see in the unit tests at the bottom of the file.
 */
@ParametersAreNonnullByDefault
@CheckReturnValue
public class MonkeyExample {
  public static class Monkey {
    public static final Lens<Monkey, Hat> Hat = lens(
        Monkey::getHat,
        (monkey, hat) -> new Monkey(monkey.name, hat));

    public static final Lens<Monkey, String> Name = lens(
        Monkey::getName,
        (monkey, name) -> new Monkey(name, monkey.hat));
  ...
  public static class Hat {
    public static final Lens<Hat, Feather> Feather = lens(
        Hat::getFeather,
        (hat, feather) -> new Hat(hat.color, feather));

    public static final Lens<Hat, Color> Color = lens(
        Hat::getColor,
        (hat, color) -> new Hat(color, hat.feather));
  ...
  public static class Feather {
    public static final Lens<Feather, Color> Color = lens(
        Feather::getColor,
        (feather, color) -> new Feather(color));
  ...
  public static class Color {
    public static final Lens<Color, String> Name = lens(
        Color::getName,
  ...
  final Monkey monkey1 =
      new Monkey(
          ...
          new Hat(
              new Color("yellow"),
              new Feather(new Color("blue"))));

  @Test
  public void testLenses() throws Exception {
    assertEquals(monkey1Json, monkey1.toJson());

    // Here's a lens that zooms all the way in from the monkey to its hat to its feather to the color of that feather!
    final Lens<Monkey, Color> monkeyHatFeatherColor =
        Monkey.Hat.andThen(Hat.Feather).andThen(Feather.Color);

    // We'll use that lens to update the color of the feather
    final Monkey monkey2 = monkeyHatFeatherColor.set(monkey1, new Color("white"));
    ...
    final Lens<Monkey, Color> monkeyHatFeatherColor2 =
        monkeyHatFeatherColor.andThen(Lens.identity());
    assertEquals(monkey2, monkeyHatFeatherColor2.set(monkey1, new Color("white")));

    // an identity lens, when "setting" a value, returns the new value, ignoring the old one entirely!
